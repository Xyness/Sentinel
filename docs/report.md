...
detection, and a terminal client for reading the results back.
...
5. A scorer putting the computed features back through that model
6. Reading the results back through a terminal client
...
A separate scorer service closes the loop between the feature store and the
model. It reads each Parquet file the streaming job closes and posts one vector
per row to the prediction endpoint, which is what makes the prediction history
reflect the live stream rather than manual calls.

A terminal client built on Rich covers the same ground in four commands:
`status` for service health and model parameters, `feed` for a running tail
of predictions, `stats` for the aggregated view with CSV export, and
`predict` for scoring a feature vector by hand.

Being a command line tool rather than a served page has two consequences
worth stating. The output composes with the usual stream tools, and a
command that cannot reach the API exits non-zero instead of drawing an
empty page, which makes it usable as a health gate in a pipeline.
...
- predictions are held in memory and do not survive an API restart.
...
from streaming data ingestion to anomaly detection and reading the results
back.
